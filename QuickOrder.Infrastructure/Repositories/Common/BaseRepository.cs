...
internal abstract class BaseRepository<TEntity>(Context context) : IRepository<TEntity> where TEntity : BaseEntity
{
    ...
    Expression<Func<TEntity, bool>>? predicate = null)
    {
        ...
        return await query.Where(predicate).OrderByDescending(x => x.Id).ToListAsync();
    }

    public virtual async Task<ICollection<TEntity>> GetWithPagination(int page, int pageSize)
    {
